...
def render_indicators(data, filtered_data, data_inicio, data_fim):
    ...
    # Renderizar indicadores como blocos clicáveis (HTML puro)
    ...
    for key, value in indicadores.items():
        ...
        if st.markdown(
            f"""
            <div class="indicador {selected_class}" style="cursor: pointer;" onclick="window.location.href='?clicked_indicator={key}'">
    ...
    if st.query_params.get("clicked_indicator"):
        st.session_state.clicked_indicator = st.query_params.get("clicked_indicator")
        st.session_state.click_count += 1
