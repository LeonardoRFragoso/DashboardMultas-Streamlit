...
def get_drive_service(credentials_json):
    try:
        if isinstance(credentials_json, str):
            credentials_json = json.loads(credentials_json)  # Certifica que está convertendo de string para dicionário

        if not isinstance(credentials_json, dict):
            raise ValueError("As credenciais fornecidas não são válidas. Certifique-se de que sejam um dicionário JSON.")
